...
上周我们用“硬编码”的小文本体验了 RAG。本周要解决一个更实际的问题：**真实文档怎么喂给 AI？**

你公司的文档不会是写在代码里的字符串，而是：
- 📄 PDF 文件（产品手册、合同）
- 📝 Markdown 文件（技术文档、Wiki）
- 🌐 HTML 文件（网页、新闻）
- 📁 JSON 文件（结构化数据）

本周你还会学到一个关键决策：**分块**。

为什么要分块？就像你不会把一整本书扔给朋友说“帮我找第 3 章的答案”，而是会直接翻到那一页。分块就是把文档拆成小段落，让检索更精准。

| 分块策略 | 一句话解释 | 适用场景 |
|---------|-----------|----------|
| 固定长度 | 每 N 个字符切一刀 | 代码、日志 |
| 递归分块 | 按段落/句子优先级切分 | 通用文本 ⭐ |
| Markdown 标题分块 | 按章节标题切分 | 结构化文档 |
| 组合策略 | 先按标题，再递归细化 | 长文档 ⭐⭐ |

> 📖 更详细的科普请阅读 `doc/Document-Processing.md`
